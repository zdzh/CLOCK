...
#define XY_READ_TIMS  10		   //读取的次数，按下一次读取多次，为了使得结果可信度更高

uint TOUCH_XPT_ReadData(uchar cmd)	   //需要使用上面的函数
{
	uchar i, j;
	...
	{	
		...
		readValue[i] = TOUCH_SPI_Read();   //读取次数多是类似于独立键盘矩阵键盘的消抖
	}
	TOUCH_CS = 1; //释放片选

	//---软件滤波---//
	//---先大到小排序，除去前两个最高值，除去后两个最低值，求其平均值---//
	//endValue为两个数字可以改变的那个中间变量
...
//	}
	...
	endValue = endValue/ (XY_READ_TIMS - 4);//求平均值，10次减去4次剩下的两次
...
uchar TOUCH_XPT_ReadXY(void)	//需要使用上面的函数
{	
	uint x1, x2, x, y1, y2, y;

	TOUCH_SPI_Start();
	//---分别读两次X值和Y值, 交叉着读可以提高一些读取精度---//
	x1 = TOUCH_XPT_ReadData(XPT_CMD_X);	   //10次是一次精度的提升，交叉着读又是一次精度的提升
	...
	xpt_xy.x = (x1 + x2) / 2;	 //真正的x的电压值
	xpt_xy.y = (y1 + y2) / 2;	 //真正的y的电压值

	xpt_xy.x &= 0xFFF0;	//去掉低四位
	xpt_xy.y &= 0xFFF0;
	 
	//---确定XY值的范围，用在触摸屏大于TFT时---//
	if((xpt_xy.x < 100) || (xpt_xy.y > 4000))	   //或
	{
		return 0;
	}
